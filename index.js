...
bot.on('message', msg => {
  ...
  switch (args[0]) {
    case 'nick':
      ...
      const botsRole = Array.from(msg.guild.roles.find(role => role.name === "bot").members)
      ...
      if (args[1] == 'random') {
          ...
          arrayID.forEach(function (ids, index, arrayID) {
            const n = msg.guild.members.get(ids).nickname || msg.guild.members.get(ids).user.username
